  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24">
      <header className="max-w-4xl">
        <p className="font-mono text-xs tracking-[0.22em] text-accent">ABOUT THE ARCHIVE</p>
        <h1 className="mt-5 text-5xl font-bold leading-[1.1] tracking-[-0.04em] text-ink sm:text-7xl">
          결론보다 과정을,
          <span className="block text-muted">확신보다 근거를 보관합니다.</span>
        </h1>
      ...
      <section className="mt-20 grid gap-5 md:grid-cols-3" aria-labelledby="principles-heading">
        ...
        {principles.map((principle) => (
          <article key={principle.number} className="rounded-sm border border-line bg-paper p-6">
            <span className="font-mono text-xs text-accent">{principle.number}</span>
            <h3 className="mt-8 text-xl font-semibold text-ink">{principle.title}</h3>
            <p className="mt-3 leading-7 text-muted">{principle.description}</p>
          </article>
        ))}
      </section>

      <section className="mt-20 grid gap-10 border-t border-line pt-12 lg:grid-cols-2">
        <div>
          <p className="font-mono text-xs tracking-[0.2em] text-muted">HOW IT WORKS</p>
          <h2 className="mt-4 text-3xl font-bold text-ink">운영 방식</h2>
        ...
        <div>
          <p className="font-mono text-xs tracking-[0.2em] text-muted">BUILT WITH</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {stack.map((item) => (
              <span key={item} className="rounded-full border border-line bg-paper px-4 py-2 text-sm text-muted">
                {item}
              </span>
            ))}
          </div>
